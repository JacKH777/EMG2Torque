...
from emg_filiter import  process_emg_data
...
class Filter():
    ...
    def filter(self, eeg_raw):
        ...
        for i in range(8): # ch1 ~ ch8
            ...
            # eeg_raw[:, i] = self.butter_bandpass_filter(eeg_raw[:, i], 5, 100, self.fs) 
            # eeg_raw[:, i] = self.butter_bandstop_filter(eeg_raw[:, i], 2, 8, self.fs)               
        ...
        eeg_raw[:, 1] = process_emg_data(eeg_raw[:, 0])
        ##################################

        return eeg_raw       
            
    def butter_bandpass(self, lowcut, highcut, fs, order=6):  # fs & order??#EEG:3，EMG:6
        nyq = 0.5 * fs
