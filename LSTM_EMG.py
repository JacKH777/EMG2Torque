...
from model import build_cnn_lstm_model, build_bidirectional_lstm_model, build_simple_rnn_model, build_lstm_model
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for emg_file, torque_file in train_files:
            ...
            if os.path.exists(emg_data_path) and os.path.exists(torque_data_path):
                ...
                if emg_data.size == 0:
                    print(emg_data_path, " is 0")
                if torque_data.size == 0:
                    print(torque_data_path, " is 0")
...
train_emg_data_list, train_torque_data_list, test_emg_data_list, test_torque_data_list = load_data_from_directory(data_directory, [], train_ratio)
...
if X_train_seq.size > 0 and X_test_seq.size > 0:
    X_train_seq = np.expand_dims(X_train_seq, axis=-1)
    X_test_seq = np.expand_dims(X_test_seq, axis=-1)
else:
    raise ValueError("Processed data arrays are empty. Check your data preprocessing steps.")
...
# 选择模型
model_name = 'lstm'  # 可以切换为 'bidirectional_lstm', 'simple_rnn', 'lstm'
...
# 训练模型
history = model.fit(X_train_seq, y_train_seq, epochs=50, batch_size=256, validation_split=0.2, callbacks=[early_stopping])
